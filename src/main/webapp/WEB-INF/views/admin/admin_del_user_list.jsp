...
<br>
	...
	&nbsp;
	<input type="button" class="btn btn-secondary" value="탈퇴회원" onclick="location.href='<%=request.getContextPath() %>/admin_del_user_list.do'" disabled>
	&nbsp;
	...
	<%-- 페이징 처리 --%>
		<div class="page-paging">
		    <c:if test="${page.startBlock > 1}"><span><a href="admin_del_user_list.do?page=1"><i class="fa fa-angle-double-left"></i></a></span></c:if>
		    <c:if test="${page.startBlock <= 1}"><span class="nolink"><i class="fa fa-angle-double-left"></i></span></c:if>
		
		    <c:if test="${page.page > 1}"><span><a href="admin_del_user_list.do?page=${page.page - 1}"><i class="fa fa-angle-left"></i></a></span></c:if>
		    ...
		    <c:if test="${page.page < page.allPage}"><span><a href="admin_del_user_list.do?page=${page.page + 1}"><i class="fa fa-angle-right"></i></a></span></c:if>
		    <c:if test="${page.page >= page.allPage}"><span class="nolink"><i class="fa fa-angle-right"></i></span></c:if>
		
		    <c:if test="${page.endBlock < page.allPage}"><span><a href="admin_del_user_list.do?page=${page.allPage}"><i class="fa fa-angle-double-right"></i></a></span></c:if>
